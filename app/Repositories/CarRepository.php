...
class CarRepository extends AbstractRepository {
    ...
    public function validateUpdate($id) {
        return $rules = [
            'title' => 'required|unique:car,title,' . $id . ',id',
            'alias' => 'required',
            'category_id' => 'required'
        ];
    }
    ...
    public function readFE($request) {
        ...
        if ($request->get('category_id')) {
            $product_ids = \DB::table('product_category')->where('category_id', $request->get('category_id'))->pluck('car_id');
            $model = $model->whereIn('id', $product_ids);
        }
        if ($request->get('attribute_id')) {
            $attribute_ids = explode(',', $request->get('attribute_id'));
            $product_ids = \DB::table('product_attribute')->whereIn('attribute_id', $attribute_ids)->pluck('car_id');
            $model = $model->whereIn('id', $product_ids);
        }
